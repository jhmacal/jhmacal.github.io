  ...
  function appendTable(parent, rows, cfg){
    ...
    rows.forEach((row, idx) => {
      ...
      row.forEach(cell => {
        ...
        const cellText = personalizeText(cell, cfg.company);
        if (cellText.includes('\n')) {
          const lines = cellText.split('\n');
          el.classList.add('om-table-cell-stacked');
          lines.forEach((line, lineIdx) => {
            const span = make('span', lineIdx === 0 ? 'om-table-cell-main' : 'om-table-cell-note', line);
            el.appendChild(span);
          });
        } else {
