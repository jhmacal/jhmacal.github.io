(function(){
  ...
  function appendTable(parent, rows, cfg){
    const wrap = make('div', 'playbook-table-wrap');
    const table = make('table', 'playbook-table');
  ...
  function appendParagraph(parent, block, cfg){
    ...
    const p = make('p', block.note ? 'playbook-note' : '');
    const annexMatch = text.match(/^Annex ([A-C])\.\s/);
    if (annexMatch) {
      p.className = 'playbook-annex-link-line';
      const a = document.createElement('a');
      a.href = '/playbook/annex-' + annexMatch[1].toLowerCase() + '/';
      a.textContent = text;
      p.appendChild(a);
    } else {
      p.textContent = text;
  ...
  function appendRequestOnlyAnnexBlock(parent, blocks, cfg){
    const group = make('div', 'playbook-request-only-block');
    group.tabIndex = 0;
    group.setAttribute('aria-label', 'Annexes available on request');
    group.appendChild(make('div', 'playbook-helper-message', 'Available on request.'));
  ...
  function renderToc(container){
    container.innerHTML = '';
    PLAYBOOK_DATA.sections.forEach(section => {
  ...
  function appendPrintCover(parent, cfg, markets){
    const cover = make('section', 'playbook-cover');
    cover.setAttribute('aria-label', 'PDF cover');
    const image = document.createElement('img');
    image.src = '/playbook/assets/ai-governance-playbook-cover.png';
    image.alt = personalizeText(PLAYBOOK_DATA.front.title, cfg.company) + ' cover';
    cover.appendChild(image);
    parent.appendChild(cover);
  }

  function appendProcessExhibit(parent){
    const exhibit = make('aside', 'playbook-exhibit playbook-exhibit--path');
    exhibit.appendChild(make('div', 'playbook-exhibit__label', 'Operating control path'));
    const flow = make('div', 'playbook-flow');
    [
      ...
      const item = make('div', 'playbook-flow__item');
  ...
  function appendTierExhibit(parent){
    const exhibit = make('aside', 'playbook-exhibit');
    exhibit.appendChild(make('div', 'playbook-exhibit__label', 'Tier consequence ladder'));
    const ladder = make('div', 'playbook-tier-ladder');
    [
      ...
      const item = make('div', 'playbook-tier-ladder__item');
  ...
  function appendGateExhibit(parent){
    const exhibit = make('aside', 'playbook-exhibit');
    exhibit.appendChild(make('div', 'playbook-exhibit__label', 'Gate sequence'));
    const gates = make('div', 'playbook-gate-map');
    [
      ...
      const item = make('div', 'playbook-gate-map__item');
  ...
  function appendRegistryExhibit(parent){
    const exhibit = make('aside', 'playbook-exhibit');
    exhibit.appendChild(make('div', 'playbook-exhibit__label', 'Registry lifecycle'));
    const lifecycle = make('div', 'playbook-registry-map');
    [
      'Proposed',
      'In review',
      'Evaluation approved',
  ...
  function renderDocument(cfg, preservePlace){
    const article = qs('playbook-document');
    if (!article) return;
    const anchor = preservePlace ? currentSectionId() : null;
    article.innerHTML = '';
    article.className = 'playbook-sheet playbook-body';
    ...
    article.appendChild(make('div', 'playbook-rule-light'));
    article.appendChild(make('div', 'playbook-document-kicker', 'Operating Model'));
    article.appendChild(make('h1', 'playbook-title', personalizeText(PLAYBOOK_DATA.front.title, cfg.company)));
    article.appendChild(make('p', 'playbook-subtitle', personalizeText(PLAYBOOK_DATA.front.subtitle, cfg.company)));

    const meta = make('div', 'playbook-meta-row');
    [['Version', PLAYBOOK_DATA.front.version.replace('Version ', '')], ['Owner', PLAYBOOK_DATA.front.owner], ['Last reviewed', PLAYBOOK_DATA.front.reviewed]].forEach(pair => {
    ...
    const config = make('div', 'playbook-print-config');
    ...
    const disclaimer = make('p', 'playbook-doc-disclaimer', personalizeText(PLAYBOOK_DATA.front.disclaimer, cfg.company));
    article.appendChild(disclaimer);

    const contents = make('nav', 'playbook-doc-contents');
    ...
    PLAYBOOK_DATA.sections.forEach(section => {
      const sec = make('section', 'playbook-section');
      sec.id = section.id;
      sec.appendChild(make('div', 'playbook-section-mark', 'Section ' + section.number));
    ...
    const foot = make('div', 'playbook-doc-identity');
    ...
    let externalActions = qs('playbook-external-actions');
    if (!externalActions && article.parentElement) {
      externalActions = make('div', 'playbook-external-actions');
      externalActions.id = 'playbook-external-actions';
      article.insertAdjacentElement('afterend', externalActions);
    }
    if (externalActions) {
      externalActions.innerHTML = '';
      const btn = make('button', 'playbook-print-button');
      btn.type = 'button';
      btn.title = 'Download the configured playbook as a clean PDF.';
      btn.setAttribute('aria-label', 'Download configured playbook as PDF');
      btn.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M12 3v11"></path><path d="m7 10 5 5 5-5"></path><path d="M5 21h14"></path></svg><span class="playbook-visually-hidden">Download configured playbook as PDF</span>';
      btn.addEventListener('click', () => window.print());
      externalActions.appendChild(btn);
    }

    updateActiveToc();
    window.dispatchEvent(new CustomEvent('playbook:rendered'));
    if (anchor) {
  ...
  function updateActiveToc(){
    const links = Array.from(document.querySelectorAll('#playbook-rail-toc a'));
    const id = currentSectionId();
    links.forEach(link => link.classList.toggle('active', link.getAttribute('href') === '#' + id));
  }

  function bindLanding(){
    const form = qs('playbook-config-form');
    if (!form) return;
    ...
    form.addEventListener('submit', event => {
      ...
      window.location.href = '/playbook/document/?' + params.toString();
    });
  }

  function bindDocument(){
    if (!qs('playbook-document')) return;
    const cfg = configFromParams();
    renderToc(qs('playbook-rail-toc'));
  ...
  bindLanding();
  bindDocument();
  window.PLAYBOOK_TEST = { data: PLAYBOOK_DATA, visible, personalizeText };
})();
